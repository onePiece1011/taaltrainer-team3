<?php
    ...
    if (isset($_GET['reset']) && $_GET['reset'] === 'true') {
        ...
        header('Location: koerdischzinnenleren.php');
        exit();
    }

    if (!isset($_SESSION['koerdisch_questions']) || !isset($_SESSION['koerdisch_question_index'])) {
        $result = $conn->query("SELECT kurdish_word, dutch_word FROM vocabulary WHERE kurdish_word IS NOT NULL AND dutch_word IS NOT NULL AND category = 'Zinnen'");
        $all = [];
...
<body>
    <main>
        <section>
            <div>
                <h1>Koerdisch zinnen oefenen</h1>
                <?php
                    ...
                    if ($finished) {
                        ...
                        echo '<p>Geen zinnen beschikbaar.</p>';
                    } else {
                        ...
                        if ($isRepeatPhase) {
                            echo '<p><strong>Herhalingsfase:</strong> Je oefent nu de zinnen die je eerder fout had.</p>';
                        }
